...
class CategorieC {
	...
	*****************ajout****************************/
	...
	/********** modifier ****************************/
	function modifierProduit($categorie,$numcat){
		$sql="UPDATE categories SET numcat=:numcatt, nomcat=:nomcat WHERE numcat=:numcat"; // modifier un emp selon cin donné
        ...
        $nomcat=$categorie->getNomcat();
		$datas = array(':numcatt'=>$numcatt,':nomcat'=>$nomcat);
		$req->bindValue(':numcatt',$numcatt);
		$req->bindValue(':numcat',$numcat);
		$req->bindValue(':nomcat',$nomcat);
	...
	{
		$sql="SELECT * from categorie where numcat=$numcat";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
